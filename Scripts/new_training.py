...
def train_adversial_agent(env, agent_to_train, agent_to_play_against, n_player=1, epochs=3000, rows=6, cols=7, sync_freq=10, display_info=False, save=False, path_to_save="", name="model_trained"):
    ...
    for i in range(1, epochs):
        ...
        if n_player == 2:
            first_action = agent_to_play_against.choose_action(1, state, env)
            state = np.reshape(env.step([first_action, None])[0]["observation"]["board"], [1, rows, cols])

        while True:
            j+=1
            
            trained_action = agent_to_train.choose_action(n_player, state, env)
            if n_player == 1:
            ...
            if not env.done:
                ...
                if n_player == 1:
                    action_against = agent_to_play_against.choose_action(2, intermediary_state, env)
                    
                    after_step = env.step([None, action_against])[0]
                else:
                    action_against = agent_to_play_against.choose_action(1, intermediary_state, env)
            ...
            if done:
                ...
                predicted_output = agent_to_train.network.forward(torch.stack(new_game_states))

                loss = agent_to_train.network.loss(predicted_output, torch.tensor(game_discounted_rewards))
                loss.backward()
