...
def test_agent(env,agent_to_test,agent_against,n_player = 1,nb_games = 100,rows = 6,cols = 7):
    ...
    for i in range(1, nb_games):
        ...
        if n_player == 2:
            first_action = agent_against.choose_action(1, state, env)
        ...
        while True:
            ...
            if not env.done:
                ...
                if n_player == 1:
                    action_against = agent_against.choose_action(2, intermediary_state, env)
                    after_step = env.step([None, action_against])[0]
                else:
                    action_against = agent_against.choose_action(1, intermediary_state, env)
